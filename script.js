document.addEventListener('DOMContentLoaded', () => {
    ...
    window.insertTextAtCursor = (text) => {
        ...
        if (sel.getRangeAt && sel.rangeCount) {
            ...
            // Apply current color to the text
            const span = document.createElement("span");
            const currentColor = document.getElementById('text-color').value;
    ...
    colorPicker.addEventListener('input', (e) => {
        restoreSelection();
        // If there is a selection, change its color
        const sel = window.getSelection();
        if (!sel.isCollapsed) {
    ...
    // Prevent losing focus when clicking the color picker label
    document.querySelector('.color-picker-container').addEventListener('mousedown', (e) => {
        // Only prevent if clicking label, let the input click through
